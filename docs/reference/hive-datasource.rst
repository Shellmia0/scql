...
- ``REPLACE(s, from, to)`` → ``regexp_replace(s, from, to)``
...
2. ``DATE_FORMAT`` uses different format patterns in MySQL (``%Y-%m-%d``) vs Hive (``yyyy-MM-dd``). Automatic conversion is planned.
3. ``ADDDATE`` / ``SUBDATE`` are not available in Hive. Use ``DATE_ADD`` / ``DATE_SUB`` instead. Automatic conversion is planned.
4. ``GROUP_CONCAT`` needs to be converted to ``concat_ws(sep, collect_list(col))`` in Hive.
5. ``LIMIT offset, count`` syntax is not supported by Hive.
