...
FALLBACK_SINGLE_PARTY_URL = "https://raw.githubusercontent.com/secretflow/scql/v1.x/cmd/regtest/testdata/single_party.json"


def resolve_query_file():
    if QUERY_FILE.exists():
        return QUERY_FILE
    fallback = Path("/tmp/single_party.json")
    if not fallback.exists():
        log(
            f"Query file not found at {QUERY_FILE}, downloading fallback single_party.json ..."
        )
        urllib.request.urlretrieve(FALLBACK_SINGLE_PARTY_URL, fallback)
    return fallback
...
def create_tables():
    ...
    for party in ("alice", "bob"):
        ...
        for table_name, table_info in data.items():
            ...
            col_defs = ", ".join(
                f"{col['column_name']} {TYPE_MAP.get(col['dtype'], 'STRING')}"
                for col in cols
            )
        ...
        for table_name, table_info in data.items():
            ...
            with csv_path.open("w") as f:
                for _ in range(row_count):
                    ...
                    f.write(",".join(values) + "\n")
            load_sql = (
                f"LOAD DATA LOCAL INPATH '{csv_path}' OVERWRITE INTO TABLE {hive_table}"
            )
...
def register_tables():
    ...
    for party, user, password in (
        ...
        for table_name, table_info in data.items():
            ...
            col_defs = ", ".join(
                f"{col['column_name']} {scql_type_map.get(col['dtype'], 'STRING')}"
                for col in cols
            )
            scql_admin(
                f"DROP TABLE IF EXISTS {scql_table}", user=user, password=password
            )
            create_sql = (
    ...
    for party, user, password in (
        ...
        for table_suffix in ("tbl_0", "tbl_1", "tbl_2"):
            ...
            code, _ = scql_admin(
                f"GRANT SELECT PLAINTEXT ON {table} TO {party}",
                user=user,
                password=password,
            )
